...
from ..layers.attention_layers import ViMBlock
from ..utils.rtdetr_utils import _init_weights, segm_init_weights

@MODELS.register_module()
class ViMDet(nn.Module):
    def __init__(
            self,
            img_size=[640, 640],
            in_channels=3,
        ...
        # mamba blocks
        self.layers = nn.ModuleList([ViMBlock(d_model=d_model,  
                                              d_state=d_state, 
                                              norm_layer_cls=nn.LayerNorm, 
                                              drop_prob=inter_dpr[i], layer_idx = i) for i in range(depth)])
    ...
    def forward(self, x, inference_params = None):
        ...
        for layer in self.layers:
              hidden_states = layer(x, inference_params)
