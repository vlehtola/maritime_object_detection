...
from ..layers.attention_layers import VSSDBlock
from ..utils.rtdetr_utils import _init_weights, segm_init_weights

@MODELS.register_module()
class VSSDDet(nn.Module):
    def __init__(
            self,
            img_size=[640, 640],
            in_channels=3,
        ...
        if self.if_efficient_fpn:
             self.eff_fpn  = EfficientFeaturePyramid(in_channels=d_model, 
                                           ...
                                           mixer_cls=partial(VSSDBlock, d_state = d_state, drop_prop = 0.5, num_heads = num_heads))
    ...
    def forward(self, x, format_output=False):
