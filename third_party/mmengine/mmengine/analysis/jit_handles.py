...
def selective_scan_flop_jit(inputs, outputs):
    # xz, conv1d_weight, conv1d_biase, x_proj_weight, delta_proj_weight, A, B, C, D, delta_bias, B_proj_weight, c_proj_bias, delta_softplus, checkpoint_lvl 
    B, D, L = inputs[0].type().sizes()
    D = D // 2
    N = inputs[5].type().sizes()[1]
    flops1 = flops_selective_scan_fn(B=B, L=L, D=D, N=N, with_D=True, with_Z=True) # flop from selective scan
    flops2 = conv_flop_count(x_shape=inputs[0].type().sizes(), w_shape=inputs[1].type().sizes(), out_shape=outputs[0].type().sizes())
